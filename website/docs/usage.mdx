...
The default configuration is designed to work with ReactDOM. If you want to try implementing React-reparenting with other renderers, you can disable the setting that automatically sends DOM nodes in the send method (since it wouldn't work without a DOM), and manually send the nodes you are working with. You can even try to [instruct](./renderer) the package to manage the types of nodes you are working with, in order to automate the process as happens with ReactDOM.
